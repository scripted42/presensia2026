...
class QrManagementController extends Controller
{
    ...
    // Download massal dengan progress dan error handling yang lebih baik
    public function downloadMassal(Request $request)
    {
        try {
            ...
            // Check if ZipArchive is available
            if (class_exists('ZipArchive')) {
                return $this->createZipDownload($students);
            } else {
                return $this->createIndividualDownloads($students);
            }
            
        } catch (\Exception $e) {
            \Log::error('Download massal error: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Terjadi kesalahan saat membuat file download. Silakan coba lagi.');
        }
    }
    
    // Create ZIP download dengan progress dan error handling
    private function createZipDownload($students)
    {
        $zip = new ZipArchive();
    ...
    // Create individual downloads page
    private function createIndividualDownloads($students)
    {
        $downloadLinks = [];
        ...
        return view('qr.download-massal', compact('downloadLinks'));
    }
