...
@section('title', 'Pengaturan Tenant - Super Admin')
    ...
    <div class="bg-white overflow-hidden shadow rounded-lg mb-6">
        <div class="px-4 py-5 sm:p-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Pengaturan Tenant: {{ $school->name }}</h1>
                    <p class="text-gray-600 mt-1">Kelola pengaturan aplikasi dan fitur untuk sekolah ini</p>
                </div>
    ...
    <div id="branding-content" class="tab-content">
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <form action="{{ route('super-admin.schools.tenant-settings.update', $school) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="file" name="app_logo" id="app_logo" accept="image/*"
                                   class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            @if($school->tenantSettings && $school->tenantSettings->app_logo)
                                <p class="mt-1 text-sm text-gray-500">Logo saat ini: {{ $school->tenantSettings->app_logo }}</p>
                            @endif
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" name="app_favicon" id="app_favicon" accept="image/*"
                                   class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            @if($school->tenantSettings && $school->tenantSettings->app_favicon)
                                <p class="mt-1 text-sm text-gray-500">Favicon saat ini: {{ $school->tenantSettings->app_favicon }}</p>
                            @endif
                        </div>
                    </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <form action="{{ route('super-admin.schools.tenant-settings.update', $school) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @php
                            $features = $school->tenantSettings->features ?? \App\Models\TenantSetting::getDefaultFeatures();
                        @endphp
                ...
                <form action="{{ route('super-admin.schools.tenant-settings.update', $school) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="color" name="primary_color" id="primary_color" required 
                                   ...
                                   value="{{ old('primary_color', $school->tenantSettings->primary_color ?? '#3B82F6') }}">
                            @error('primary_color')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="color" name="secondary_color" id="secondary_color" required 
                                   class="block w-full h-12 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                   value="{{ old('secondary_color', $school->tenantSettings->secondary_color ?? '#1E40AF') }}">
                        ...
                        <div>
                            ...
                            <input type="color" name="accent_color" id="accent_color" 
                                   class="block w-full h-12 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                   value="{{ old('accent_color', $school->tenantSettings->accent_color ?? '#F59E0B') }}">
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="flex space-x-4">
                            <div class="w-16 h-16 rounded-lg border" id="primary-preview" style="background-color: {{ $school->tenantSettings->primary_color ?? '#3B82F6' }}"></div>
                            <div class="w-16 h-16 rounded-lg border" id="secondary-preview" style="background-color: {{ $school->tenantSettings->secondary_color ?? '#1E40AF' }}"></div>
                            <div class="w-16 h-16 rounded-lg border" id="accent-preview" style="background-color: {{ $school->tenantSettings->accent_color ?? '#F59E0B' }}"></div>
                        </div>
                    </div>
