...
@section('title', 'Absensi - Presensia')
    ...
    <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('attendance.check-in') }}" class="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors font-medium">
                    <i class="fas fa-sign-in-alt mr-2"></i>Absensi Masuk
                </a>
                @elseif(!$attendances->check_out)
                <a href="{{ route('attendance.check-out') }}" class="bg-red-600 text-white px-6 py-3 rounded-lg hover:bg-red-700 transition-colors font-medium">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="p-5">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <i class="fas fa-sign-in-alt text-2xl {{ $attendances && $attendances->check_in ? 'text-green-600' : 'text-gray-400' }}"></i>
                </div>
    ...
    <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <i class="fas fa-sign-out-alt text-2xl {{ $attendances && $attendances->check_out ? 'text-red-600' : 'text-gray-400' }}"></i>
                </div>
                <div class="ml-5 w-0 flex-1">
                    <dl>
    ...
    <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <i class="fas fa-calendar-check text-2xl {{ $attendances ? 'text-blue-600' : 'text-gray-400' }}"></i>
                </div>
                <div class="ml-5 w-0 flex-1">
                    <dl>
            ...
            <div class="px-4 py-3 border-t border-gray-200 sm:px-6">
                <div class="flex items-center justify-between">
                    <div class="flex-1 flex justify-between sm:hidden">
                        ...
                        @else
                            <a href="{{ $recent->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                Sebelumnya
                            </a>
                        @endif
                        
                        @if($recent->hasMorePages())
                            <a href="{{ $recent->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                Selanjutnya
                            </a>
                        @else
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Menampilkan
                                <span class="font-medium">{{ $recent->firstItem() }}</span>
                                sampai
                                <span class="font-medium">{{ $recent->lastItem() }}</span>
                                dari
                                <span class="font-medium">{{ $recent->total() }}</span>
                                hasil
                            </p>
                        </div>
                        <div>
