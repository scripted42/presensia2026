...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mb-6 desktop-stats-grid">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-3.5 sm:p-4 shadow-sm flex flex-col justify-between card-hover">
        <div class="flex items-center justify-between">
            ...
            <p class="text-xl md:text-2xl font-bold text-gray-900">{{ $stats['total_employees'] }}</p>
        </div>
        <p class="text-sm text-gray-500 font-normal mt-2.5">Total Pegawai</p>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-3.5 sm:p-4 shadow-sm flex flex-col justify-between card-hover">
        <div class="flex items-center justify-between">
            ...
            <p class="text-xl md:text-2xl font-bold text-gray-900">{{ $stats['total_students'] }}</p>
        </div>
        <p class="text-sm text-gray-500 font-normal mt-2.5">Total Siswa</p>
    </div>
    @endif
        ...
        <div class="flex items-center justify-between">
            ...
            @if($isManager)
                <div class="text-right">
                    <p class="text-xl md:text-2xl font-bold text-gray-900 leading-tight">{{ $stats['today_attendance_percent'] ?? 0 }}%</p>
                    <p class="text-xs text-gray-400 mt-0.5">{{ $stats['today_attendance'] ?? 0 }}/{{ $stats['today_total_users'] ?? 0 }}</p>
                </div>
            @endif
        </div>
        <div class="mt-2.5">
            <p class="text-sm text-gray-500 font-normal">Absensi Hari Ini</p>
            @if(!$isManager)
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-3.5 sm:p-4 shadow-sm flex flex-col justify-between card-hover">
        <div class="flex items-center justify-between">
            ...
            <p class="text-xl md:text-2xl font-bold text-gray-900">{{ $stats['my_students'] }}</p>
        </div>
        <p class="text-sm text-gray-500 font-normal mt-2.5">Siswa Saya</p>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-3.5 sm:p-4 shadow-sm flex flex-col justify-between card-hover">
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                <p class="text-xl md:text-2xl font-bold text-gray-900 leading-tight">{{ $stats['pending_leaves_percent'] ?? 0 }}%</p>
                <p class="text-xs text-gray-400 mt-0.5">{{ $stats['pending_leaves'] ?? 0 }}/{{ $stats['pending_leaves_total'] ?? 0 }}</p>
            </div>
        </div>
        <p class="text-sm text-gray-500 font-normal mt-2.5">Izin Pending</p>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-3.5 sm:p-4 shadow-sm flex flex-col justify-between card-hover">
        <div class="flex items-center justify-between">
            ...
            <p class="text-xl md:text-2xl font-bold text-gray-900">{{ $stats['approved_leaves'] }}</p>
        </div>
        <p class="text-sm text-gray-500 font-normal mt-2.5">Izin Disetujui</p>
    </div>
    @endif
</div>

<!-- Section 1: Ringkasan Performa (4 stat cards full-width) -->
...
<div class="mb-6">
    <div class="flex items-center justify-between mb-3">
        <h2 class="text-lg font-semibold text-gray-900">Ringkasan Performa</h2>
    ...
    <div id="date-sheet" class="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl z-50 transform translate-y-full transition-transform duration-300 shadow-2xl safe-bottom max-h-[85vh] overflow-y-auto">
        ...
        <div class="px-6 pb-8">
            ...
            <form method="GET" action="{{ route('dashboard') }}" id="bottom-sheet-date-form">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-medium text-gray-500 mb-1.5">Filter Peran</label>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="perf-stat-card group relative bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
            <div class="flex items-start justify-between mb-2">
                <span class="text-sm text-gray-500 font-normal">Penggunaan</span>
            ...
            <p class="text-xl md:text-2xl font-bold {{ $getDaisyColor($usagePct) }} leading-none mb-1">
                {{ $formatPercent($usagePct) }}
            </p>
            <p class="text-xs text-gray-500">{{ $activeUsers }} dari {{ $totalUsers }} pengguna absen</p>
            ...
            <div class="perf-tooltip-popup w-72 bg-gray-900 text-white rounded-xl shadow-2xl p-4 text-xs leading-relaxed">
                ...
                <div class="mt-2 text-xs text-gray-400">Filter aktif: <span class="text-blue-300 font-semibold">{{ $filterLabel }}</span> · Hari libur dikecualikan</div>
                <!-- Arrow -->
        ...
        <div class="perf-stat-card group relative bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
            <div class="flex items-start justify-between mb-2">
                <span class="text-sm text-gray-500 font-normal">KPI Absensi</span>
            ...
            <p class="text-xl md:text-2xl font-bold {{ $getDaisyColor($kpiScore) }} leading-none mb-1">
                {{ $formatPercent($kpiScore) }}
            </p>
            <p class="text-xs text-gray-500">Skor gabungan 4 indikator kunci</p>
            ...
            <div class="perf-tooltip-popup w-80 bg-gray-900 text-white rounded-xl shadow-2xl p-4 text-xs leading-relaxed">
                ...
                <div class="bg-gray-800 rounded-lg p-2 text-xs text-gray-400">
                ...
                <div class="mt-1.5 text-xs text-gray-500">≥80% Baik · 50–79% Perlu Perhatian · &lt;50% Kritis</div>
                <div class="tooltip-arrow"></div>
            </div>
        </div>
        ...
        <div class="perf-stat-card group relative bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
            <div class="flex items-start justify-between mb-2">
                <span class="text-sm text-gray-500 font-normal">Data Pegawai</span>
            ...
            <p class="text-xl md:text-2xl font-bold {{ $getDaisyColor($empPct) }} leading-none mb-1">
                {{ $formatPercent($empPct) }}
            </p>
            <p class="text-xs text-gray-500">{{ $empComplete }} dari {{ $empTotal }} profil lengkap</p>
            ...
            <div class="perf-tooltip-popup w-72 bg-gray-900 text-white rounded-xl shadow-2xl p-4 text-xs leading-relaxed">
                ...
                <div class="text-xs text-gray-500">Cek tab "Profil Tidak Lengkap" di Perlu Ditindaklanjuti ↓</div>
                <div class="tooltip-arrow"></div>
            </div>
        </div>
        ...
        <div class="perf-stat-card group relative bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
            <div class="flex items-start justify-between mb-2">
                <span class="text-sm text-gray-500 font-normal">Data Siswa</span>
            ...
            <p class="text-xl md:text-2xl font-bold {{ $getDaisyColor($stuPct) }} leading-none mb-1">
                {{ $formatPercent($stuPct) }}
            </p>
            <p class="text-xs text-gray-500">{{ $stuComplete }} dari {{ $stuTotal }} profil lengkap</p>
    ...
    <!-- Legenda warna -->
    <div class="flex items-center gap-4 mt-2.5 px-1 flex-wrap">
        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>≥80% Baik</span>
        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="inline-block w-2 h-2 rounded-full bg-amber-400"></span>50–79% Perhatian</span>
        <span class="flex items-center gap-1.5 text-xs text-gray-500"><span class="inline-block w-2 h-2 rounded-full bg-red-500"></span>&lt;50% Kritis</span>
        <span class="ml-auto text-xs text-gray-400 italic">Arahkan kursor ke kartu untuk detail</span>
    </div>
</div>
@endif

<!-- Section 2: Perlu Ditindaklanjuti (Full Width dengan Tabs) -->
@if(isset($metrics) && ($user->hasRole('admin') || $user->hasRole('headmaster') || $user->hasRole('bk') || $user->hasRole('kesiswaan')))
@php
...
<div class="mb-6">
    <div class="flex items-center justify-between mb-3">
        <h2 class="text-lg font-semibold text-gray-900">Perlu Ditindaklanjuti</h2>
    </div>
    ...
    <div class="rounded-2xl border border-gray-100 bg-white p-5 md:p-6 shadow-sm">
        ...
        <div id="zona2-panel-all" class="zona2-panel divide-y divide-gray-100">
            ...
            <div class="flex items-center justify-between py-3.5 hover:bg-gray-50/50 px-2 rounded-xl transition-colors">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">{{ $row['subtitle'] }}</p>
                </div>
                <div>
                    <span class="badge {{ $row['badge_class'] }} badge-sm text-xs font-medium">
                        {{ $row['badge_text'] }}
                    </span>
                </div>
            </div>
        ...
        <div class="pt-4 border-t border-gray-100 text-center">
            <button type="button" onclick="openZona2Modal()" id="zona2-view-all-btn" class="text-xs font-medium text-blue-600 hover:text-blue-800 transition-colors inline-flex items-center gap-1">
...
<div id="zona2-modal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-black bg-opacity-40 flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl relative max-h-[85vh] flex flex-col">
        ...
        <div class="pt-3 border-t border-gray-100 flex justify-end">
            <button type="button" onclick="closeZona2Modal()" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium text-xs rounded-xl transition-colors">Tutup</button>
        </div>
    </div>
</div>
@endif

<!-- Section 3: Baris Baru 3 Kolom Sejajar: Absensi Terbaru | Terlambat Hari Ini | Sedang Izin/Cuti -->
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 items-stretch">
    <!-- Kolom 1: Absensi Terbaru (Attendance Feed dengan Live Update) -->
    <div class="flex flex-col h-full">
        <div class="flex items-center justify-between mb-3 px-1 flex-shrink-0">
            ...
            <div class="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200/60 shadow-xs">
                ...
                <span class="text-xs text-emerald-700 font-medium tracking-tight">Live update</span>
            </div>
        </div>
        
        <div class="card bg-white border border-gray-100 shadow-sm rounded-2xl flex-1 flex flex-col h-[400px]">
                            ...
                            <div class="flex items-center gap-2.5 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="flex items-center gap-1.5 text-xs text-gray-500 mt-0.5 flex-wrap">
                                        ...
                                        <span>•</span>
                                        <span class="truncate max-w-[110px] text-gray-400" title="{{ $feed['location'] }}">
                                            {{ $feed['location'] }}
                                        </span>
                                        @endif
                                    </div>
    ...
    <div class="flex flex-col h-full">
        <div class="flex items-center justify-between mb-3 px-1 flex-shrink-0">
            <h2 class="text-lg font-semibold text-gray-900">Terlambat Hari Ini</h2>
            <span id="late-today-badge" class="badge badge-warning badge-sm text-xs font-medium {{ (isset($lateToday) && $lateToday->isNotEmpty()) ? '' : 'hidden' }}">{{ isset($lateToday) ? $lateToday->count() : 0 }}</span>
        </div>

        <div class="card bg-white border border-gray-100 shadow-sm rounded-2xl flex-1 flex flex-col h-[400px]">
            <div class="card-body p-4 md:p-5 flex-1 flex flex-col min-h-0">
                <div id="late-today-list" class="flex-1 overflow-y-auto divide-y divide-gray-100 -mx-1 px-1 pr-1.5 custom-feed-scrollbar min-h-0">
                    @if(isset($lateToday) && $lateToday->isNotEmpty())
                        ...
                        <div class="flex items-center justify-between py-2.5 hover:bg-gray-50/50 rounded-xl px-1.5 transition-colors">
                            <div class="flex items-center gap-2.5 min-w-0">
                                ...
                                @else
                                <div class="avatar placeholder flex-shrink-0">
                                    <div class="bg-amber-50 text-amber-600 rounded-full w-8 h-8 text-xs font-bold flex items-center justify-center border border-amber-100">
    ...
    <div class="flex flex-col h-full">
        <div class="flex items-center justify-between mb-3 px-1 flex-shrink-0">
            <h2 class="text-lg font-semibold text-gray-900">Sedang Izin/Cuti</h2>
            <span id="on-leave-badge" class="badge badge-neutral badge-sm text-xs font-medium {{ (isset($onLeaveToday) && $onLeaveToday->isNotEmpty()) ? '' : 'hidden' }}">{{ isset($onLeaveToday) ? $onLeaveToday->count() : 0 }}</span>
        </div>

        <div class="card bg-white border border-gray-100 shadow-sm rounded-2xl flex-1 flex flex-col h-[400px]">
            <div class="card-body p-4 md:p-5 flex-1 flex flex-col min-h-0">
                <div id="on-leave-list" class="flex-1 overflow-y-auto divide-y divide-gray-100 -mx-1 px-1 pr-1.5 custom-feed-scrollbar min-h-0">
                    @if(isset($onLeaveToday) && $onLeaveToday->isNotEmpty())
                        ...
                        <div class="flex items-center justify-between py-2.5 hover:bg-gray-50/50 rounded-xl px-1.5 transition-colors">
                            <div class="flex items-center gap-2.5 min-w-0">
                                ...
                                @else
                                <div class="avatar placeholder flex-shrink-0">
                                    <div class="bg-purple-50 text-purple-600 rounded-full w-8 h-8 text-xs font-bold flex items-center justify-center border border-purple-100">
